...
class Helper
{
    ...
    /**
     * Get asset URL - Environment aware (works with both root and public document roots)
     */
    public static function asset($path)
    {
        ...
        // Auto-detect if running from public directory
        $docRoot = $_SERVER['DOCUMENT_ROOT'] ?? '';
        if (basename($docRoot) === 'public') {
            // Running with: php -S localhost:8000 -t public
            return $basePath . '/assets/' . $cleanPath;
        }

        // Running with: php -S localhost:8000 (from root)
        return $basePath . '/public/assets/' . $cleanPath;
    }

    /**
     * Get upload URL - Environment aware (works with both root and public document roots)
     */
